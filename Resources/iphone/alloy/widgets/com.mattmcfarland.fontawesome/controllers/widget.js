...
function Controller() {
    ...
    function addIcon(tag, iconChar) {
        ...
        if (tag["font"]) {
            tag["font"]["fontSize"] && (fSize = tag["font"]["fontSize"]);
            tag["font"]["iconFontSize"] && (fSize = tag["font"]["iconFontSize"]);
            tag["font"]["iconPosition"] && "append" == tag["font"]["iconPosition"] && (position = tag["font"]["iconPosition"]);
        }
        if ("append" == position) {
            tag["text"] && (aText = tag["text"] + " " + aText);
            tag["title"] && (aTitle = tag["title"] + " " + aTitle);
        } else {
