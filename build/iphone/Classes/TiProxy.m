...
//Rare exceptions to indicate a bug in the _myfoot code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_myfoot = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_myfoot]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.myfoot.module"
